function [header headerascii]=readVB17Header(fname);
% function headers=read_dat_headers(fname);
%
...
fid=fopen(fname,'r');
if fid == -1
    errordlg('Could not open header file');
    header = -1;
    return;
end
...
%if nargout==2
    ...
    if isfield(headerascii, 'Phoenix');     header.Phoenix.part1      = Config2struct(    regexprep(headerascii.Phoenix, '([^#]*)(.*)', '$1')); end     % Phoenix enth�lt zwei Teile, also beide Strukturtypen kommen vor
    ...
    % pehses: teilweise verschluckt sich das skript leicht ...
    if isfield(headerascii, 'com');         header.Dicom              = Config2struct(    headerascii.com); end
    if isfield(headerascii, 'eas');         header.Meas               = Config2struct(    headerascii.eas); end
    if isfield(headerascii, 'aps');         header.MeasYaps           = MeasYaps2struct(  headerascii.aps); end
...
%     ***************************************
%
% Das ist bisher eine SEHR rudiment�re Implementierung!
...
% besitzen kann es unter Umst�nden vorkommen, dass Werte �berschrieben
% werden!
%
% Bessere Alternative:
% 
...
% welches �hnlich wie ein xml-File aufgebaut ist. Das sog. EvaProtocol kann
...
% kann. Die Strings liegen hier aber in headers auch schon vor. Man k�nnte
...
%% ParamLong:
    ...
    for m=1:size(matches,2)
        ...
        if ~isempty(strfind(Wert, ' '))
            ...
            Wert = regexprep(Wert, ' +', ' ');
            % Klammern hinzuf�gen, damit die Werte bei eval() als Array gefressen werden:
            Wert = ['[' Wert ']'];
        end
...
%function [Variable, Wert] = clearVariableWert(Variable, Wert)
    ...
    % Falls sich ein Wert �ber mehrere Zeilen erstreckt sollten sie noch
